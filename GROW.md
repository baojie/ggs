...
bash wiki/scripts/skill_commit.sh "phase4/4-A: Lint 扫描完成（P2=7, P3=15）"
```
...
- [x] P1 问题：无（待 H1 重型基因确认后复查）
- [x] 系统性 LNT2 标点问题：根因为原著格式，无需模板修订
- [ ] FIX8 类型一致性 7 候选：待人工复核后确定是否为系统性模板问题
- [x] `logs/phase4/fix-summary.md` 已生成（P2=7 暂缓, P3=15 无需修复）
...
- [x] 所有已建立类型均有 `logs/phase4/evv6-<type>-*.md` 日志
- [x] 无类型触发模板修订（所有 schema 定义充分，执行缺口在 enrich 覆盖率）
- [x] 类型反思汇总表填写完整：

| 类型 | 质量均分 | Phase 4 EVV6 | 模板修订 | 主要扣分模式 |
|------|---------|-------------|---------|------------|
| concept | 39.0/40 | 97.5% | 否 | 13 standard + 1 basic 待 enrich |
| species | 28.0/40 | 70.0% | 否 | 25 basic 页 PN=0，51% 未 enrich |
| place | 40.0/40 | 100% | 否 | 全部 featured，无问题 |
| person | 35.6/40 | 89.0% | 否 | 7 standard + 2 basic 待 enrich |
| event | 40.0/40 | 100% | 否 | 全部 featured，无问题 |
...
- [x] `SCN23` 扫描已执行，结果写入 `logs/phase4/type-discovery.md`
- [x] 结论：搁置（现有五类型体系充分，无新类型成立）
...
- [x] 空 wikilink 已清零（LNT14 无问题）
- [x] 112 处不可解析断链记录在案（多指向未建页面或误写目标名，排入 Phase 5 修复）
- [x] Wikify dry-run 审查未执行（章节页无新增 alias 变更，无需重新 wikify）
- [x] backlinks.json 已重建（218 页被引用，1199 条反向链接）
...
- [x] `logs/phase4/lint-audit.md` — P0/P1=0，H1 重型基因已记录在案并排入 Phase 5
- [x] 所有类型完成 EVV6，日志存在（species/person/concept/place/event）
- [x] 均分 < 70 的类型（species 28.0）模板评估为充分，无需修订
- [x] 新类型勘探结论明确（type-discovery.md 存在）
- [x] Wikify 完成，backlinks 索引已重建
- [x] 页面总数未增加（291→291，验证无误建页）
- [x] `grow_phase4_summary.md` 存在，包含质量对比数据
- [ ] `butler.json` 中 `grow_phase` 已更新为 `5`（H1 待执行，保持 4）
...
print(f'  模板修订    : 0')
print(f'  新类型发现  : 0（延至 Phase 5）')
print(f'  Wikify 链接 : 0 条新增')
print('=' * 60)
