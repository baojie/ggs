...
- [x] 创建配套检测脚本：`wiki/scripts/lint_block_format.py`
- [x] 执行全库扫描并自动修复：
  ```bash
  python3 wiki/scripts/lint_block_format.py --dir docs/wiki/pages/ --fix
  ```
  - 结果：扫描 24 文件，**0 处问题**（全库 ::: 区块格式已符合规范，无需修复）
- [x] 验证所有章节页面无残留问题
  - 结果：LNT15 全部 6 类检查均为 0 处
- [x] 确认本次提交不含 `:::` 区块格式问题
...
**目标**：推导该 wiki 的实体类型体系，调整 `docs/wiki/local/config/types.js` 配置，为后续批量建页和分类查询建立类型先验。
...
| type | 说明 | 预估数量 | 优先级 |
|------|------|---------|--------|
| `concept` | 核心理论概念（地理决定论、粮食生产等） | ~80 | P1 |
| `person` | 历史人物、学者、原住民代表 | ~50 | P1 |
| `place` | 地理区域、大陆、文明地点 | ~60 | P1 |
| `species` | 动植物、病原体 | ~80 | P1 |
| `event` | 历史事件（驯化、征服等） | ~30 | P2 |
| `chapter` | 章节摘要（已有 22 章） | 22 | 已完成 |
| `overview` | 综述/目录页 | ~3 | 已有 |

**输出验证**：`logs/butler/type-survey.md` 存在且包含至少 3 种类型及其估算数量。

### 7-B 类型体系配置调整

当前 `docs/wiki/local/config/types.js` 已有类型：
`concept / person / event / organization / place / species / overview / chapter / list`

LAW.md §四 定义的类型：
`concept / person / chapter / place / event / species / overview`
...
2. - [x] 执行 **QUO7**（pn-format-lint）——对新建页面执行 PN 格式检查并自动修复：
   ```bash
   python3 "$MEMEX_ROOT/wiki/scripts/butler/pn_format_lint.py" li/粮食生产 --fix
   ```
   - 结果：✓ `li/粮食生产.md` 无问题（0 处违规，无需修复）

3. - [x] 执行 **CHK7**（new-page-system-check）：
   - S1 ✓ 页面渲染正常
...
4. - [x] CHK7 实质通过（S5/S6 脚本时序误报，Playwright 直接验证正常）→ 进入 9-B
...
每种类型按以下流程执行三轮迭代，所有步骤均须人工审核（⏸）后方可进入下一步：

```
Phase 9 开始：R=0，在 local/BIRTH.md 记录

对每个类型 T（按依赖顺序：concept → species → place → person → event）：
...
每种类型共消耗 **8 轮**（SCN27×3 + EVV5×3 + EVV6×1 + QUO23×1），R 累加。若 EVV6 判断为 `needs-work`，可追加第 4 轮迭代，但通常 3 轮已足够收敛。

**类型执行顺序**（先建基础类型，后建依赖它们的类型）：
...
在 `local/BIRTH.md` Phase 9 节维护进度表，**只记录轮次状态，不记录具体页面**。具体建了哪些页、选页理由、PN 来源，全部记录在各轮 SCN27 日志文件中（`logs/gene-express/`）。
...
|------|---------|---------|---------|---------|---------|-------|---------|------|-----------|------|
| concept | R1 ✓ | R2 ✓ | R3 ✓ | R4 ✓ | R5 ✓ | R6 ✓ | R7 ✓ | R8 ✓ | R8 ✓ | 完成 |
| species | R9 ✓ | R10 ✓ | R11 ✓ | R12 ✓ | R13 ✓ | R14 ✓ | R15 ✓ | R16 ✓ | R16 ✓ | 完成 |
| place | R17 ✓ | R18 ✓ | R19 ✓ | R20 ✓ | R21 ✓ | R22 ✓ | R23 ✓ | R24 ✓ | R24 ✓ | 完成 |
| person | R25 ✓ | R26 ✓ | R27 ✓ | R28 ✓ | R29 ✓ | R30 ✓ | R31 ✓ | R32 ✓ | R32 ✓ | 完成 |
| event | R33 ✓ | R34 ✓ | R35 ✓ | R36 ✓ | R37 ✓ | R38 ✓ | R39 ✓ | R40 ✓ | R40 ✓ | 完成 |
...
读完全部类型后，横向归纳（写入 local/memory/boot_summary.md §Pilot 信息组织核心洞察）：
  · 跨类型共同收敛的结构规律
  · 最频繁被调整的内容维度
  · 类型间共性与最大差异点
...
对 10-B 中判定需要 RFC 的每个目标文件，执行：

1. 在 `ref/rfc/` 下创建 `rfc-ggs-NNNN-slug.md`（status: proposed）
2. 执行 `/rfc ref/rfc/rfc-ggs-NNNN-slug.md` 提交为 memex GitHub issue
3. 将 issue URL 写回 RFC 文件的 Issue 字段

不同文件的修改意见对应不同 RFC，一次可连续提交多个。

- [ ] 复盘 RFC 已按需提交（或确认无需提交）
...
bash wiki/scripts/skill_commit.sh "docs: boot summary for Phase 9 — 枪炮、病菌与钢铁"
```

- [ ] `local/memory/boot_summary.md` 已写入并 commit
...
> `{}` 内的占位符均取自 10-A 汇总表与 `boot_summary.md` 中的数据，逐项填入真实值再打印。
...
- [ ] Butler 实例命名已确认（或用户选择跳过），`local/config.md` 已更新
...
1. 在本 wiki 的 `ref/rfc/` 下创建 `rfc-ggs-NNNN-slug.md`（status: proposed）
