...
- [ ] 确认 `pn_prefix` 已写入所有目标章节（Phase 5-B 完成）
- [ ] 阅读 `$MEMEX_ROOT/ref/spec/data-pn.md §2`，了解通用跳过规则
- [ ] 记录本 wiki 的额外跳过元素（如 sidebar 注释、epub 脚注定义、特殊块类型等），写入本 BIRTH.md 5-C-0 节
- [ ] 确认或创建本地 PN 赋号脚本
...
- [ ] 对试验章单独执行 PN 标注（`$MEMEX_ROOT/skills/gene/PRE7-chapter-pn-assign.md`）
- [ ] 本地渲染验证：`./wiki-daemon.sh start`，在浏览器打开该章节页面，检查：
...
- [ ] 严格按 `ref/chapter-order.md` 的 NNN 次序，逐章执行 PN 标注
- [ ] 每章完成后验证编号连续性（无跳号），再推进下一章
- [ ] 非正文章节（前言、附录、参考文献等）同样赋号，不得跳过
- [ ] 全量完成后再次整体验证跨章连续性（`QUO10-cross-chapter-pn-verify`）
- [ ] 执行 `$MEMEX_ROOT/ref/spec/workflow-post-pn-lint.md` 定义的完整 post-PN lint 流程
