...
[指向本书哪章/哪节有更集中的论述，含 1–2 处 PN 引注。标准档必填。]
```
...
> ⚠️ **写作规范**：禁止使用破折号（——）作句间连接符（如"X——这正是Y"）。配对括注（"开篇——引文——提炼"）可接受，但避免超过 1 对。建页后若出现连接符式破折号，须改为句号或逗号。
...
| standard | + 在本书中的角色节 + 主要论点节 + ≥3 PN + 相关概念 Wikilink + 延伸阅读节 |
| featured | + ≥6 PN + 引文规范全符合 + 无连接符式破折号 |
